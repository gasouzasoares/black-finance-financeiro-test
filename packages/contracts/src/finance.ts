...
export const listSchema=z.object({from:day,to:day,account_id:id.optional(),legal_entity_id:id.optional(),direction:z.enum(directions).optional(),group:z.enum(groups).optional(),party_id:id.optional(),category_id:id.optional(),cost_center_id:id.optional(),label_id:id.optional(),focus:z.enum(['today','overdue']).optional(),status:z.enum(['open','settled','cancelled']).optional(),q:z.string().max(100).optional(),sort:z.enum(['due_on','title','amount_minor']).optional(),order:z.enum(['asc','desc']).optional(),cursor:z.string().max(1600).optional(),limit:z.coerce.number().int().min(1).max(200).default(50)}).refine(v=>v.to>=v.from && Date.parse(v.to)-Date.parse(v.from)<=366*86400000,{message:'Escolha um período de até 12 meses.'});
