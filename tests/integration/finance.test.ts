...
test('financial core: exact ledger, atomicity, concurrency, scoped APIs and 1000 commands',async t=>{
 ...
 const editFixture=await f.entry(ctx(),input());
 const patchEntry=(version:string,extra:Record<string,string>={})=>app.inject({method:'PATCH',url:`/v1/entries/${editFixture.id}`,headers:{...ownHeaders,'x-entity-version':version,'idempotency-key':randomUUID(),...extra},payload:{...input(),title:'Versão revisada pela API'}});
 assert.equal((await patchEntry('1')).statusCode,200);
 assert.equal((await patchEntry('1')).statusCode,412);
 assert.equal((await patchEntry('abc')).statusCode,400);
 assert.equal((await patchEntry('2',{'if-match':'1'})).statusCode,400);
