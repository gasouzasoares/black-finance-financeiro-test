...
// Vercel rewrites add a `path` query key; only consume the application fields.
export const driveFolderQuery=z.object({folder:driveId.optional()});
export const driveChunkQuery=z.object({offset:z.coerce.number().int().min(0).max(20000000)});
export function driveRoutes(app:FastifyInstance,f:Finance,context:(r:FastifyRequest)=>Promise<Context>){
 ...
 app.get('/v1/drive/folders',async r=>s.browse(await context(r),parse(driveFolderQuery,r.query).folder));
 ...
 app.get('/v1/drive/files/:id/chunk',async r=>s.chunk(await context(r),rid(r),parse(driveChunkQuery,r.query).offset));
