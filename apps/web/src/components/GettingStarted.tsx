...
export function GettingStarted({ navigate, can }: { navigate: (page: string) => void; can: (resource: string, action?: string) => boolean }) {
  const steps = [
    ...
    { title: 'Registre o pagamento', description: 'No detalhe do lançamento, use Liquidar e informe a data em que o dinheiro entrou ou saiu. Isso atualiza o saldo.', page: 'entries', resource: 'entries', icon: CheckCircle2, action: 'Ver lançamentos' },
    { title: 'Confira o resultado', description: 'Em Contas e extratos, abra Ver extrato. Compare o movimento e o saldo após a operação.', page: 'accounts', resource: 'accounts', icon: BookOpen, action: 'Conferir extrato' },
  ];
