...
try{
 ...
 const name='20260915000200_internal_modules.sql',sql=await readFile(`supabase/migrations/${name}`,'utf8'),hash=createHash('sha256').update(sql).digest('hex');
 ...
 await db.query('COMMIT');console.log(previous?'Estrutura já atualizada.':'Estrutura de módulos internos aplicada.');
